...
import { desaAman, u1Photos } from "@/lib/catalogue/desa-aman";
export function DesaAmanHouse() {
  return <main className="living-page"><div className="living-container house-detail">
    ...
    <div className="room-grid">{desaAman.rentalRooms.map(name=><Link className="room-card" key={name} href={`/units/desa-aman/rooms/${name.toLowerCase()}`}>
      <div className={`room-card-image ${name === "U1" ? "" : "room-placeholder"}`}>{name === "U1" ? <><Image src={u1Photos[1].src} alt="Room U1 with a queen bed and study desk" fill sizes="(max-width:760px) 100vw, 33vw" className="object-cover" /><span className="photo-label"><Box size={14} /> Interactive 3D tour</span></> : <><span>{name}</span><small>Photos coming soon</small></>}</div>
      <div className="room-card-copy"><div><h3>Room {name}</h3><ArrowUpRight size={20} /></div><p>{name.startsWith("U") ? "Upper" : "Ground"} floor{name === "U1" ? " · Queen bed · Air conditioning" : " · Rental room"}</p><span className="room-status">Confirm availability</span></div>
    </Link>)}</div></section>
  </div></main>;
}
