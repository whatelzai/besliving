...
type ModelApi = { preset: (value: Preset) => void; zoom: (factor: number) => void; rotate: (angle: number) => void; lighting: (warm: boolean) => void };
...
export default function G2Model() {
  ...
  useEffect(() => {
    ...
    scene.add(sunlight);
    const lamp = new THREE.PointLight(0xffc274, 0, 5); lamp.position.set(-1,1.6,0.3); scene.add(lamp);
    ...
    function sphere(parent:THREE.Object3D,x:number,y:number,z:number,rx:number,ry:number,rz:number,m:THREE.Material) {
      const mesh=new THREE.Mesh(new THREE.SphereGeometry(1,16,10),m);mesh.position.set(x,y,z);mesh.scale.set(rx,ry,rz);mesh.castShadow=true;parent.add(mesh);return mesh;
    }
    function plant(parent:THREE.Object3D,x:number,y:number,z:number,scale=1) {
      const pot=new THREE.Mesh(new THREE.CylinderGeometry(.065*scale,.048*scale,.12*scale,16),white);pot.position.set(x,y+.06*scale,z);parent.add(pot);
      for(let i=0;i<7;i++){const a=i*Math.PI*2/7;const leaf=sphere(parent,x+Math.cos(a)*.07*scale,y+.18*scale+(i%2)*.045*scale,z+Math.sin(a)*.07*scale,.045*scale,.10*scale,.02*scale,green);leaf.rotation.z=Math.cos(a)*.6;leaf.rotation.y=-a;}
    }
    ...
    for(let z=-.48;z<.9;z+=.25)for(let y=.55;y<1.35;y+=.24)sphere(bed,-1.348,y,z,.007,.012,.012,white);
    box(bed,-.02,.605,.17,1.28,.035,1.53,throwMat,true);
    for(const z of [-.59,.93])box(bed,-.02,.39,z,1.28,.41,.022,throwMat);
    for(const z of [-.20,.53]){const p=box(bed,-1.03,.70,z,.42,.20,.65,bedding,true);p.rotation.z=-.2;}
    for(const [z,m] of [[-.20,material("#bb943f")],[.53,white]] as const){const p=box(bed,-.96,.83,z,.16,.38,.43,m,true);p.rotation.z=.25;}
    box(bed,.02,.65,.12,.55,.045,.40,white,true);plant(bed,-.10,.68,.09,.85);
    box(bed,.16,.69,.18,.19,.025,.14,linen);
    const desk=feature("desk");
    box(desk,-.96,.76,-2.10,1.05,.045,.55,paleWood);
    for(const x of [-1.42,-.50])for(const z of [-2.31,-1.88])box(desk,x,.38,z,.035,.75,.035,white);
    box(desk,-1.15,.80,-2.07,.33,.02,.23,metal,true);
    box(desk,-.72,.80,-2.09,.22,.015,.17,dark,true);
    const clock=new THREE.Mesh(new THREE.CylinderGeometry(.105,.105,.045,24),dark);clock.rotation.x=Math.PI/2;clock.position.set(-.62,.91,-2.27);desk.add(clock);
    box(desk,-.96,.45,-1.55,.42,.055,.42,white,true);
    box(desk,-.96,.68,-1.33,.43,.40,.055,white,true);
    for(const x of [-1.13,-.79])for(const z of [-1.71,-1.37])box(desk,x,.23,z,.028,.44,.028,white);
    const wardrobe=feature("wardrobe");
    box(wardrobe,1.59,.97,1.10,1.03,1.94,.53,wood);
    for(const x of [1.33,1.85])box(wardrobe,x,1.23,.823,.50,1.35,.025,paleWood);
    for(let i=0;i<3;i++){box(wardrobe,1.85,.15+i*.18,.822,.49,.17,.03,paleWood);box(wardrobe,1.85,.15+i*.18,.80,.13,.015,.022,metal);}
    for(const x of [1.53,1.65])box(wardrobe,x,1.10,.80,.018,.15,.025,metal);
    const windowGroup=feature("window");
    for(let i=0;i<37;i++)box(windowGroup,-1.46+i*.078,1.22,D/2-.07+(i%2)*.028,.086,2.40,.07,curtain,true);
    const nookCurtains=new THREE.Group();windowGroup.add(nookCurtains);
    for(let i=0;i<13;i++)box(nookCurtains,-1.45+i*.079,1.22,-D/2-.89+(i%2)*.025,.086,2.4,.06,curtain,true);
    box(walls.left,-W/2+.13,2.48,-.59,.23,.34,.92,white,true);
    for(let i=0;i<5;i++)box(walls.left,-W/2+.25,2.38+i*.025,-.59,.009,.007,.81,metal);
    const ropeMat=new THREE.MeshStandardMaterial({color:0xffebbd,emissive:0xffbc58,emissiveIntensity:1.6});materials.add(ropeMat);
    ...
    const shadowGround=new THREE.Mesh(new THREE.PlaneGeometry(200,200),material("#e8e5dc"));shadowGround.rotation.x=-Math.PI/2;shadowGround.position.y=-.18;shadowGround.receiveShadow=true;scene.add(shadowGround);
    let interior=false;
    function render(){
    ...
    function setPreset(value:Preset){
      ...
      if(value==="plan"){camera.position.set(0,7.8,.001);controls.target.set(0,0,0);}
      else if(interior){camera.position.set(1.08,1.70,-1.15);controls.target.set(-.55,.95,.30);}
      else{camera.position.set(5.0,5.7,-6.7);controls.target.set(0,.75,0);}
    ...
    const resize=new ResizeObserver(()=>{const w=container.clientWidth,h=container.clientHeight;if(w&&h){renderer.setSize(w,h);camera.aspect=w/h;camera.updateProjectionMatrix();render();}});resize.observe(container);
    let startX=0,startY=0;
    const pointerDown=(e:PointerEvent)=>{startX=e.clientX;startY=e.clientY;};
    const pointerUp=(e:PointerEvent)=>{if(Math.hypot(e.clientX-startX,e.clientY-startY)>6)return;const r=renderer.domElement.getBoundingClientRect();const ray=new THREE.Raycaster();ray.setFromCamera(new THREE.Vector2((e.clientX-r.left)/r.width*2-1,-(e.clientY-r.top)/r.height*2+1),camera);const hit=ray.intersectObjects(objects,true)[0];if(hit){let o:THREE.Object3D|null=hit.object;while(o&&!o.userData.feature)o=o.parent;if(o)setSelected(features[o.userData.feature]);}};
    const lost=(e:Event)=>{e.preventDefault();setFailed(true);};
    renderer.domElement.addEventListener("pointerdown",pointerDown);
    renderer.domElement.addEventListener("pointerup",pointerUp);
    renderer.domElement.addEventListener("webglcontextlost",lost);
    setPreset("overview");
    return ()=>{api.current=null;resize.disconnect();controls.dispose();renderer.domElement.removeEventListener("pointerdown",pointerDown);renderer.domElement.removeEventListener("pointerup",pointerUp);renderer.domElement.removeEventListener("webglcontextlost",lost);scene.traverse(o=>{if(o instanceof THREE.Mesh)o.geometry.dispose();});materials.forEach(m=>m.dispose());renderer.dispose();renderer.domElement.remove();};
  },[]);

  return <div className="model-shell">
    <div className="model-presets" aria-label="Camera views">{([['overview','Dollhouse'],['plan','Top view'],['inside','Inside']] as const).map(([value,label])=><button key={value} aria-pressed={preset===value} onClick={()=>{setPreset(value);api.current?.preset(value);}}>{label}</button>)}</div>
    <div className="model-canvas" ref={host} tabIndex={0} aria-label="G2 3D room. Drag to rotate, scroll to zoom, or use arrow keys and plus or minus." onKeyDown={e=>{if(['ArrowLeft','ArrowRight','+','-'].includes(e.key)){e.preventDefault();if(e.key==='ArrowLeft')api.current?.rotate(-.15);if(e.key==='ArrowRight')api.current?.rotate(.15);if(e.key==='+')api.current?.zoom(.85);if(e.key==='-')api.current?.zoom(1.15);}}} />
    {failed && <div className="model-unavailable" role="status"><h3>3D isn’t available on this device.</h3><p>You can still explore G2 using the Photos and Floor plan tabs above.</p></div>}
    <span className="model-badge">G2 <span>DESA AMAN</span></span>
    <div className="model-tools"><button title="Zoom in" aria-label="Zoom in" onClick={()=>api.current?.zoom(.85)}><Plus size={18}/></button><button title="Zoom out" aria-label="Zoom out" onClick={()=>api.current?.zoom(1.15)}><Minus size={18}/></button><button title="Reset view" aria-label="Reset view" onClick={()=>{setPreset("overview");api.current?.preset("overview");}}><RotateCcw size={17}/></button><button aria-label={warm?"Use daylight":"Use warm lighting"} aria-pressed={warm} onClick={()=>{setWarm(!warm);api.current?.lighting(!warm);}}>{warm?<Sun size={18}/>:<Moon size={18}/>}</button></div>
    <div className="model-help"><Move size={15}/><span>Drag to rotate · Scroll or pinch to zoom</span></div>
    <div className="model-features">{Object.keys(features).map(key=><button key={key} onClick={()=>setSelected(features[key])}>{key === "window" ? "Window & AC" : key[0].toUpperCase()+key.slice(1)}</button>)}</div>
    <p className="model-selection" aria-live="polite">{selected}</p>
  </div>;
}
