...
export default function U1Model() {
  ...
  useEffect(() => {
    ...
    for(let x=-W/2;x<W/2;x+=.6)box(room,x,.012,0,.007,.002,D,grout);
    for(let z=-D/2;z<D/2;z+=.6)box(room,0,.012,z,W,.002,.007,grout);
    ...
    for(let x=-1.26;x<.1;x+=.25){for(let y=.55;y<1.25;y+=.24)sphere(bed,x,y,D/2-.218,.013,.013,.007,white);}
    box(bed,-.64,.599,.08,1.53,.035,1.25,dark,true);
    box(bed,-1.385,.39,.08,.023,.41,1.25,dark);
    box(bed,.105,.39,.08,.023,.41,1.25,dark);
    for(const x of [-1.01,-.29]){const p=box(bed,x,.72,1.19,.66,.22,.43,white,true);p.rotation.x=.25;}
    const cushion=box(bed,-.63,.81,.97,.44,.36,.14,material("#b38a47"),true);cushion.rotation.x=-.25;
    box(bed,-.55,.64,.12,.57,.04,.38,white,true);
    box(bed,-.65,.67,.08,.22,.035,.15,material("#2a7778"));
    const cup=new THREE.Mesh(new THREE.CylinderGeometry(.037,.033,.07,18),white);cup.position.set(-.4,.695,.15);bed.add(cup);
    const desk=feature("desk");
    box(desk,-.99,.77,-1.06,.77,.055,1.00,paleWood);
    for(const x of [-1.31,-.67])for(const z of [-1.48,-.64])box(desk,x,.38,z,.045,.76,.045,white);
    ...
    for(const x of [-.56,-.25])for(const z of [-1.19,-.88])box(desk,x,.23,z,.035,.44,.035,white);
    const wardrobe=feature("wardrobe");
    box(wardrobe,.92,.95,-1.28,.96,1.90,.52,wood);
    for(let i=0;i<3;i++){box(wardrobe,.60+i*.32,.98,-1.006,.31,1.79,.035,paleWood);box(wardrobe,.71+i*.25,.91,-.976,.017,.18,.022,metal,true);}
    ...
    for(let i=0;i<23;i++){const fold=box(windowGroup,-1.30+i*.064,1.13,-1.54+(i%2)*.035,.076,2.18,.09,curtain,true);fold.rotation.y=(i%2 ? .16 : -.16);}
    box(windowGroup,-.57,2.27,-1.52,1.60,.045,.055,metal,true);
    box(windowGroup,-.57,2.52,-1.47,1.12,.35,.23,white,true);
    for(let i=0;i<8;i++)box(windowGroup,-.57,2.42+i*.026,-1.341,1,.007,.008,metal);
    // Shelves and plants from the room photos, attached to the feature wall.
    for(const y of [1.42,1.95]){box(walls.left,-1.30,y,-.93,.28,.032,.67,white);plant(walls.left,-1.25,y+.02,-1.1,.8);plant(walls.left,-1.26,y+.02,-.75,.65);}
    ...
    const rope=new THREE.Mesh(new THREE.TubeGeometry(new THREE.CatmullRomCurve3(ropePoints),64,.009,6,false),ropeMat);walls.left.add(rope);
    ...
    function setPreset(value:Preset){
      ...
      else if(interior){camera.position.set(1.05,1.70,1.22);controls.target.set(-.3,1.1,-.6);}
      else{camera.position.set(4.6,4.7,5.7);controls.target.set(0,.75,0);}
  ...
  return <div className="model-shell">
    ...
    <div className="model-canvas" ref={host} tabIndex={0} aria-label="U1 3D room. Drag to rotate, scroll to zoom, or use arrow keys and plus or minus." onKeyDown={e=>{if(['ArrowLeft','ArrowRight','+','-'].includes(e.key)){e.preventDefault();if(e.key==='ArrowLeft')api.current?.rotate(-.15);if(e.key==='ArrowRight')api.current?.rotate(.15);if(e.key==='+')api.current?.zoom(.85);if(e.key==='-')api.current?.zoom(1.15);}}} />
    {failed && <div className="model-unavailable" role="status"><h3>3D isn’t available on this device.</h3><p>You can still explore U1 using the Photos and Floor plan tabs above.</p></div>}
    <span className="model-badge">U1 <span>DESA AMAN</span></span>
